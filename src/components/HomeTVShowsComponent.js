...
function HomeTVShowsComponent({ contents }) {
    ...
    const checkContent = async (index) => {
        try {
            const response = await fetch('/userList/check', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleListClick = async (index) => {
        if (removeRef.current[index].style.display === 'none') {
            ...
            addRef.current[index].style.display = 'none';
            try {
                const response = await fetch('/userList/add', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            try {
                const response = await fetch('/userList/remove', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    return (
        <>
            <div ref={homeContainerRef} style={{ marginLeft: '50px' }}>
                <h1 style={{ color: 'white', fontFamily: 'NetflixSansLite', position: 'sticky', top: '90px', zIndex: '100', paddingTop: '10px' }}>TV Shows</h1>
                <br></br>
...
export default HomeTVShowsComponent;